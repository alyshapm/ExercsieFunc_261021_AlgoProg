def calcNewHeight():
    ...
    while newWidth > currentWidth:
        newWidth = eval(input("Re-enter desired width, must be smaller than current width: ")) # new width must be smaller than current width so while loop is used so user inputs value that meets requirement
